...
function DiagnosticDashboard() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      if (!existingDiag.find(d => d.email === parsed.email)) {
        localStorage.setItem("registeredDiagnostics", JSON.stringify([...existingDiag, parsed]));
      }
    }

    // Load scan requests for this center
    const requests = JSON.parse(localStorage.getItem("diagnosticRequests")) || [];
    const storedProfile = stored ? JSON.parse(stored) : {};
    setScanRequests(requests.filter(r => r.center === storedProfile.centerName));
  ...
  const handleFiles = async (incoming) => {
    ...
    try {
      ...
      if (targetRequest) {
        ...
        setScanRequests(updatedReqs.filter(r => r.center === diagnostic.centerName));
        setTargetRequest(null);
      }
    } catch (error) {
      alert("Error saving file. It might be too large for local storage.");
    }
  };
            ...
            <aside className="right">
              ...
              <div style={{ marginTop: 8 }}>
                ...
                <div className="activity-timeline">
                  {files.slice(-4).reverse().map((f, i) => (
                    <div className="activity-item" key={f.id || i}>
                      <div className="activity-dot-wrap"><div className="activity-dot" /></div>
                      <div className="activity-body">
                        <div className="activity-title">{f.patientEmail || "Patient"}</div>
                        <div className="activity-badge" style={{ background: '#eef2ff', color: '#2563eb' }}>UPLOAD</div>
                        <div className="activity-desc">Uploaded <strong>{f.name}</strong></div>
                        <div className="activity-time">🕐 {f.date}</div>
                      </div>
                    </div>
                  ))}
                  {files.length === 0 && (
                    <div style={{ textAlign: 'center', padding: '24px 0', color: '#6b7a99', fontSize: '0.82rem' }}>
                      No recent activity
                    </div>
                  )}
                </div>
                  ...
                  <div className="doc-settings-header">
                    ...
                    <div className="doc-settings-actions">
                      ...
                      <button className="doc-btn-primary" onClick={() => {
                        ...
                        const existingDiag = JSON.parse(localStorage.getItem("registeredDiagnostics")) || [];
                        const idx = existingDiag.findIndex(d => d.email === diagnostic.email);
                        if (idx >= 0) existingDiag[idx] = newProfile;
