...
/* ─── Read-only Calendar (Dashboard tab) ─── */
const ReadOnlyCalendar = ({ doctorName }) => {
  ...
  useEffect(() => {
    ...
    const mine = saved.filter(a => a.doctor === doctorName);
    const map = {};
    mine.forEach(a => {
      const d = new Date(a.date);
      const key = `${d.getFullYear()}-${d.getMonth() + 1}-${d.getDate()}`;
      if (!map[key]) map[key] = [];
      map[key].push({ time: a.time, patient: a.patientName });
    });
    setApptMap(map);
  }, [doctorName]);
  ...
  return (
    <div className="card" style={{ marginTop: 24 }}>
      <h3 className="card-title" style={{ marginBottom: 2 }}>Appointment Calendar</h3>
      <p className="card-sub" style={{ fontSize: "0.76rem", marginBottom: 14 }}>View-only — manage from the Appointments tab</p>
        ...
        {cells.map((d, i) => {
          ...
          return (
            <div key={i} className={`cal-cell ${isToday ? "today" : ""} ${appts.length ? "has-appt" : ""}`} style={{ cursor: "default" }}>
              ...
              {appts.map((a, idx) => (
                <div key={idx} className={`appt-chip ${isPast ? "appt-chip-past" : "appt-chip-upcoming"}`} style={{ pointerEvents: "none" }}>
...
const PatientDetailView = ({ patient, doctor, onBack }) => {
  ...
  /* Load patient data */
  const allRx = JSON.parse(localStorage.getItem("allPrescriptions")) || [];
  const myRx = allRx.filter(p => p.patientName === patient.patientName);
  ...
  const centers = (() => {
    ...
    const all = [...ex];
    if (solo && solo.centerName && !all.find(d => d.email === solo.email)) all.push(solo);
    return all;
  })();
                ...
                {myRx.slice().reverse().map((h, i) => (
                  <div key={i} style={{ background: "#f8fafc", borderRadius: 10, padding: "11px 14px", borderLeft: "3px solid #2e6db4" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
                      ...
                      <span style={{ fontSize: "0.69rem", color: "#8a96b0" }}>{h.date}</span>
                    </div>
...
const PatientListView = ({ patients, doctor, onSelect }) => {
  ...
  const allRx = JSON.parse(localStorage.getItem("allPrescriptions")) || [];
  const logCountByPatient = {};
  allRx.filter(p => isToday(p.date) && p.doctor === doctor?.name).forEach(p => {
    logCountByPatient[p.patientName] = (logCountByPatient[p.patientName] || 0) + 1;
  });

  const allAppts = JSON.parse(localStorage.getItem("allAppointments")) || [];
  const apptCountByPatient = {};
  allAppts.filter(a => isToday(a.date) && a.doctor === doctor?.name).forEach(a => {
    apptCountByPatient[a.patientName] = (apptCountByPatient[a.patientName] || 0) + 1;
  });
  ...
  patients.forEach(p => {
    if (!seen.has(p.patientName)) {
      const logsToday = logCountByPatient[p.patientName] || 0;
      const apptsToday = apptCountByPatient[p.patientName] || 0;
      const isCompletedToday = logsToday > 0 && logsToday >= apptsToday;
            ...
            <>
              <div className="page-header" style={{ marginBottom: 24 }}>
                <div className="greeting">
                  <h2>Welcome, Dr. {doctor.name?.split(" ")[0] || "Doctor"}.</h2>
              ...
              <ReadOnlyCalendar doctorName={doctor.name} />
            </>
          )}
              ...
              <div className="doc-settings-header" style={{ marginBottom: 24 }}>
                ...
                <div className="doc-settings-actions">
                  ...
                  <button className="doc-btn-primary" onClick={() => {
                    ...
                    setDoctor(np); localStorage.setItem("doctorProfile", JSON.stringify(np)); alert("Profile saved!");
                  }}>Save Changes</button>
                </div>
              </div>
